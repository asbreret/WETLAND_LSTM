...
#        model.add(LSTM(numHiddenUnits, return_sequences=True, activation=lstm_activation))
       
        model.add(LSTM(units=numHiddenUnits, 
                    return_sequences=True, 
                    activation=lstm_activation,
                    kernel_regularizer=L1(0.001),  # L1 regularization on weights
                    recurrent_regularizer=L1(0.001)  # L1 regularization on recurrent weights
                    )) 
       
        # model.add(Bidirectional(LSTM(units=numHiddenUnits, 
        #                     return_sequences=True, 
        #                     activation='tanh', 
        #                     kernel_regularizer=L1L2(l1=0.001, l2=0.001),  # Combined L1 and L2 on weights
        #                     recurrent_regularizer=L1L2(l1=0.001, l2=0.001)  # Combined L1 and L2 on recurrent weights
        #                     )))
    ...
    # Compile the model
    model.compile(loss='mse', optimizer=optimizer_obj, metrics=['mae', 'mse'])
